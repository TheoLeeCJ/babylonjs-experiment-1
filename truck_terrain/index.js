...
BABYLON.SceneLoader.Load("./", "toyota_dyna_compressed.glb", engine, (loadedScene) => {
	// Assign
	...
	{ // SCENE DEFINITION + LIGHTING
		...
		// ground = BABYLON.Mesh.CreateGroundFromHeightMap("Ground", "/assets/textures/heightMap.png", 128, 128, 64, 0, 10, carScene, false, () => {
		// 	ground.material = terrainMaterials[0];
		// });
		...
		// ground2 = BABYLON.Mesh.CreateGroundFromHeightMap("Ground", "/assets/textures/heightMap.png", 128, 128, 64, 0, 10, carScene, false, function () {
		...
		// 	ground2.material = terrainMaterials[0];
		// });
		...
		// Environment
		var hdrTexture = new BABYLON.CubeTexture.CreateFromPrefilteredData("./environment-out.env", carScene);
		...
		skysphereMtl.lightmapTexture = new BABYLON.Texture("./environment-out.png");
		...
		sun.intensity = 2;
		sun.position = new BABYLON.Vector3(100, 8, 100);
	...
	if (sceneReady) {
		if (!shadowMapPopulated) {
			...
			for (i = 0; i < carScene.meshes.length; i++) {
				if (!carScene.meshes[i].name.includes("Ground")) {
